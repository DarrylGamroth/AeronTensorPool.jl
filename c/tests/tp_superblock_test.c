...
int main(void)
{
    ...
    shm_tensorpool_control_shmRegionSuperblock_wrap_for_encode(
        &sb,
        (char *)buffer,
        0,
        sizeof(buffer));

    const uint64_t magic = 0x544F504C53484D31ULL;
    shm_tensorpool_control_shmRegionSuperblock_set_magic(&sb, magic);
