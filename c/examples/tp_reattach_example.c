...
int main(int argc, char **argv)
{
    ...
    env = getenv("TP_REATTACH_DELAY_MS");
    uint64_t delay_ms = env ? (uint64_t)strtoull(env, NULL, 10) : 1000;
    ...
    if (tp_producer_reattach(&producer) != TP_OK)
    {
        fprintf(stderr, "producer reattach failed\n");
    ...
    printf("reattached producer for stream %u\n", stream_id);
    tp_producer_close(producer);
    tp_client_close(client);
    tp_context_close(ctx);
    return 0;
