@testset "Consumer waits for matching stream announce after producer revoke" begin
    with_driver_and_client() do driver, client
        mktempdir("/dev/shm") do dir
            ...
            stream_id = UInt32(4243)
            other_stream = UInt32(4244)
            ...
            AeronTensorPool.Hsm.dispatch!(state.driver_lifecycle, :AttachOk, state)
                ...
                @test Consumer.map_from_announce!(state, announce.dec, announce_ts)
                ...
                Consumer.handle_driver_events!(state, announce_ts)
                @test state.awaiting_announce_epoch == epoch
                @test state.phase == AeronTensorPool.UNMAPPED

                fetch!(state.clock)
                bad_ts = UInt64(Clocks.time_nanos(state.clock))
                announce_other = build_shm_pool_announce(
                ...
                @test !Consumer.handle_shm_pool_announce!(state, announce_other.dec)
                @test state.awaiting_announce_epoch == epoch
                @test state.phase == AeronTensorPool.UNMAPPED
