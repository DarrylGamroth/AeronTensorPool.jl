@testset "Consumer ndims range validation" begin
    with_driver_and_client() do driver, client
        mktempdir("/dev/shm") do dir
            ...
            try
                ...
                function write_and_check(seq::UInt64, ndims::UInt8)
                    ...
                    safe_ndims = ndims > UInt8(MAX_DIMS) ? UInt8(MAX_DIMS) : ndims
                    write_slot_header!(
                        slot_enc,
                        tensor_enc,
                        UInt64(0),
                    ...
                    if ndims != safe_ndims
                        header_pos = SlotHeaderMsg.sbe_offset(slot_enc) +
                            SlotHeaderMsg.sbe_block_length(SlotHeaderMsg.Decoder) +
                            SlotHeaderMsg.headerBytes_header_length
                        tensor_override = TensorHeaderMsg.Encoder(Vector{UInt8})
                        TensorHeaderMsg.wrap!(tensor_override, header_mmap, header_pos)
                        TensorHeaderMsg.ndims!(tensor_override, ndims)
                    end
                ...
                write_and_check(UInt64(1), UInt8(0))
                write_and_check(UInt64(2), UInt8(MAX_DIMS + 1))
            finally
