@testset "Consumer validation helpers" begin
    @test validate_stride(UInt32(4096); require_hugepages = false)
    @test !validate_stride(UInt32(5000); require_hugepages = false)
    @test !validate_stride(UInt32(4096); require_hugepages = true)
