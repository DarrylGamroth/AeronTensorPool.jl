...
@testset "Discovery expiry" begin
    ...
    @test !AeronTensorPool.entry_expired(entry, UInt64(150))
    @test AeronTensorPool.entry_expired(entry, UInt64(250))

    entry.expiry_timer.last_ns = UInt64(0)
    @test AeronTensorPool.entry_expired(entry, UInt64(100))
