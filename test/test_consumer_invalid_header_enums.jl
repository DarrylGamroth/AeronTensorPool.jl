...
@testset "Consumer drops invalid dtype/major_order" begin
    with_driver_and_client() do driver, client
        mktempdir("/dev/shm") do dir
            nslots = UInt32(8)
            stride = UInt32(4096)
            layout_version = UInt32(1)
            stream_id = UInt32(205)
            epoch = UInt64(1)
            ...
            try
                ...
                function write_and_check(dtype::Dtype.SbeEnum, order::MajorOrder.SbeEnum)
                ...
                write_and_check(Dtype.UNKNOWN, MajorOrder.ROW)
                write_and_check(Dtype.UINT8, MajorOrder.UNKNOWN)
            finally
