# AeronTensorPool.jl

AeronTensorPool.jl implements the SHM Aeron Tensor Pool v1.2 spec in Julia.

- Wire spec reference: `docs/SHM_Tensor_Pool_Wire_Spec_v1.2.md`
- Driver model spec: `docs/SHM_Driver_Model_Spec_v1.0.md`
- Discovery service spec: `docs/SHM_Discovery_Service_Spec_v_1.0.md`
- Bridge spec: `docs/SHM_Aeron_UDP_Bridge_Spec_v1.0.md`
- Bridge config example: `docs/examples/bridge_config_example.toml`
- RateLimiter spec: `docs/SHM_RateLimiter_Spec_v1.0.md`
- RateLimiter config example: `docs/examples/rate_limiter_config_example.toml`
- Implementation guide: `docs/IMPLEMENTATION_GUIDE.md`
- Implementation notes: `docs/IMPLEMENTATION.md`
- Implementation phases: `docs/IMPLEMENTATION_PHASES.md`
- Operational guidance: `docs/OPERATIONAL_PLAYBOOK.md`
- Integration examples: `docs/INTEGRATION_EXAMPLES.md`
- User guide: `docs/USER_GUIDE.md`

## API

```@autodocs
Modules = [AeronTensorPool]
```
