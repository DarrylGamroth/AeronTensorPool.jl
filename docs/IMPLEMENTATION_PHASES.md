...
Status
- Complete (mapping, seqlock read, mode handling, drops accounting with max_outstanding_seq_gap resync, header validation including nslots power-of-two and epoch checks, commit_word frame_id consistency, announce-time superblock revalidation with PID change handling, fallback handling).
